...
const maps = {
  ...
  fortress: [
    { x: 100, y: 100, radius: 30, type: "standing", broken: false },
    { x: 700, y: 100, radius: 30, type: "standing", broken: false },
    { x: 100, y: 500, radius: 30, type: "standing", broken: false },
    { x: 700, y: 500, radius: 30, type: "standing", broken: false },
    { x: 400, y: 300, radius: 35, type: "standing", broken: false },
  ],
  maze: [
    { x: 200, y: 150, radius: 20, type: "standing", broken: false },
    { x: 400, y: 100, radius: 20, type: "standing", broken: false },
    { x: 600, y: 150, radius: 20, type: "standing", broken: false },
    { x: 150, y: 300, radius: 20, type: "standing", broken: false },
    { x: 350, y: 250, radius: 20, type: "standing", broken: false },
    { x: 550, y: 300, radius: 20, type: "standing", broken: false },
    { x: 200, y: 450, radius: 20, type: "standing", broken: false },
    { x: 400, y: 500, radius: 20, type: "standing", broken: false },
    { x: 600, y: 450, radius: 20, type: "standing", broken: false },
  ],
  open: [],
};
...
function showMapSelection() {
  ...
  ctx.fillText("SELECT MAP", canvas.width / 2, 100);

  ctx.font = "20px Arial";
  ctx.fillText("Press 1-4 to select:", canvas.width / 2, 150);

  ctx.font = "16px Arial";
  ctx.fillText("1 - RUINS (Scattered pillars)", canvas.width / 2, 200);
  ctx.fillText("2 - FORTRESS (Corner defense)", canvas.width / 2, 230);
  ctx.fillText("3 - MAZE (Complex layout)", canvas.width / 2, 260);
  ctx.fillText("4 - OPEN FIELD (No cover)", canvas.width / 2, 290);
...
document.addEventListener("keydown", (e) => {
  if (!game.mapSelected) {
    if (e.key === "1") selectMap("ruins");
    else if (e.key === "2") selectMap("fortress");
    else if (e.key === "3") selectMap("maze");
    else if (e.key === "4") selectMap("open");
    return;
  }
  ...
  if (num >= 1 && num <= 6 && weapons[num - 1].unlockWave <= game.wave) {
...
document.addEventListener(
  "keyup",
  (e) => (game.keys[e.key.toLowerCase()] = false)
);
...
function updatePlayer() {
  ...
  if (
    game.keys[" "] &&
    Date.now() - game.lastShot > weapons[game.currentWeapon].fireRate
  ) {
    shoot();
...
function shoot() {
  ...
  if (weapon.name === "Shotgun") {
    ...
    const speed = weapon.explosive ? 6 : weapon.piercing ? 12 : 8;
...
function updateBullets() {
  game.bullets = game.bullets.filter((bullet) => {
    ...
    // Check pillar collisions (piercing bullets ignore pillars)
    if (!bullet.piercing) {
      for (const pillar of game.pillars) {
...
function updateGame() {
  ...
  if (
    ...
    Date.now() - game.player.lastDamageTime > 3000
  ) {
  ...
  if (game.waveActive && game.waveZombiesSpawned < getWaveZombieCount()) {
    ...
    if (game.swarmTimer % 300 < 60) {
      // 1 second swarm window
  ...
  if (
    game.waveActive &&
    game.waveZombiesKilled >= getWaveZombieCount() &&
    game.bosses.length === 0 &&
    game.bossesSpawned >= game.wave
  ) {
    ...
    if (game.wave === 2) {
      ...
      game.unlockText = "RAIL GUN UNLOCKED! Press 6";
      game.unlockTimer = 300;
    }
...
function startWave() {
  ...
  if (game.wave >= 10) {
    const railWaves = game.wave - 10;
    weapons[5].maxAmmo = 5 + railWaves * 2; // Rail Gun: 5/7/9...
  }
...
function render() {
  ...
  const armSway = game.player.isMoving
    ? Math.sin(game.player.walkCycle * 0.5) * 0.5
    : 0;
  ctx.fillStyle = "#4a90e2";
